...
\begin{answer}
	If you solve the diff. equation: $\dot{z}=-\tau\alpha_z z, \quad z(t)= -\tau \alpha_z e^{\lambda t}=-\tau \alpha_z e^{-\tau \alpha_z} \overset{t \rightarrow \infty}{=}0 \rightarrow f_w(z) \overset{t \rightarrow \infty}{=}0 $.
	...
	$G(s)=\frac{Y(s)}{g(s)}= \frac{\tau^2 \alpha \beta}{s^2 + s \tau \alpha + \tau^2 \alpha \beta },\quad s \rightarrow 0,\quad Y(s)=g(s)$
	
	There is no steady state error if your input is a 1 then also the output will be a one. This means your equilibrium point is: $y_{eqp}=g (goal)$.
	\end{answer}
