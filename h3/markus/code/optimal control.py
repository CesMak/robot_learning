...
def run_system_Kt(s_des, K_t, k_t):
    ...
    for t in range(T):
        r[t] = reward(s[:, t].reshape(2, 1), a[t], t)
        s[:, t + 1] = next_state(s[:, t].reshape(2, 1), a[t], t)
        a[t + 1] = control_signal(s[:, t + 1].reshape(2, 1), s_des[:, t + 1].reshape(2, 1), t + 1, K_t[:, t + 1],
                                  k_t[t + 1])
...
def main():
    ...
    for i in range(nr_runs):
        s4[:, :, i], a4[:, i], r4[:, i] = run_system_Kt(np.zeros((2, T + 1)), K_t,
                                                        k_t)  # run_system_Kt(s_des, K_t, k_t)
